...
/**
 * SmartSupplyApp.java — with theme toggle, enhanced login UI, and MySQL persistence.
 *
 * DATABASE SCHEMA (created automatically by the app if missing):
 *
 ...
 * );
 *
 * IMPORTANT: add MySQL JDBC driver (mysql-connector-java) to your classpath.
 */
public class SmartSupplyApp {
    // In-memory data stores (kept in sync with DB)
    ...
    // --- DB config: set these for your environment ---
    ...
    private void start() {
        // initialize DB (create tables if missing) and load products
        try {
            initDatabase();
            loadProductsFromDB();
            loadLedgerFromDB();
            ...
            ex.printStackTrace();
            // If DB fails, still continue with in-memory samples
        }
    ...
    private void addSampleProduct(Product p) {
        ...
        String ledgerEntry = timestamp() + " - REGISTERED - " + p.id + " by " + p.manufacturer;
        ledger.add(ledgerEntry);

        // persist to DB
        try {
            upsertProductToDB(p);
            addLedgerEntryToDB(ledgerEntry);
    ...
    private void openSupportChatDialog() {
        ...
        reportBtn.addActionListener(e -> {
            ...
            if (pid != null && products.containsKey(pid.trim())) {
                ...
                p.flagged = true;
                String t = timestamp() + " - FLAGGED BY CUSTOMER";
                p.timeline.add(t);
                ...
                try {
                    upsertProductToDB(p);
                    addLedgerEntryToDB(ledger.get(ledger.size() - 1));
                } catch (SQLException ex) {
                    ex.printStackTrace();
                }
    ...
    private void initDatabase() throws SQLException, ClassNotFoundException {
        // load driver (optional for modern drivers, but explicit is safe)
        ...
        try (Connection c = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS);
            ...
            String createProducts = "CREATE TABLE IF NOT EXISTS products (" +
                    ...
                    "last_updated TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP" +
                    ")";
            st.execute(createProducts);

            String createLedger = "CREATE TABLE IF NOT EXISTS ledger (" +
                    "id BIGINT AUTO_INCREMENT PRIMARY KEY," +
                    "entry TEXT," +
                    "created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP" +
                    ")";
            st.execute(createLedger);
        }
    }
    ...
    private void upsertProductToDB(Product p) throws SQLException {
        String sql = "INSERT INTO products (id, name, batchNo, manufacturer, distributor, retailer, status, assignedDelivery, lat, lon, flagged, timeline) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) " +
                "ON DUPLICATE KEY UPDATE name=VALUES(name), batchNo=VALUES(batchNo), manufacturer=VALUES(manufacturer), " +
                "distributor=VALUES(distributor), retailer=VALUES(retailer), status=VALUES(status), assignedDelivery=VALUES(assignedDelivery), " +
                "lat=VALUES(lat), lon=VALUES(lon), flagged=VALUES(flagged), timeline=VALUES(timeline)";
        try (Connection c = getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, p.id);
            ps.setString(2, p.name);
            ps.setString(3, p.batchNo);
            ps.setString(4, p.manufacturer);
            ps.setString(5, p.distributor);
            ps.setString(6, p.retailer);
            ps.setString(7, p.status);
            ps.setString(8, p.assignedDelivery);
            ps.setDouble(9, p.lat);
            ps.setDouble(10, p.lon);
            ps.setBoolean(11, p.flagged);
            ps.setString(12, String.join("\n", p.timeline));
            ps.executeUpdate();
        }
    }

    private void addLedgerEntryToDB(String entry) throws SQLException {
        String sql = "INSERT INTO ledger (entry) VALUES (?)";
        try (Connection c = getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
    ...
    private void loadProductsFromDB() throws SQLException {
        String sql = "SELECT id, name, batchNo, manufacturer, distributor, retailer, status, assignedDelivery, lat, lon, flagged, timeline FROM products ORDER BY last_updated ASC";
        try (Connection c = getConnection();
             PreparedStatement ps = c.prepareStatement(sql);
            ...
            while (rs.next()) {
                ...
                p.batchNo = rs.getString("batchNo");
                p.manufacturer = rs.getString("manufacturer");
                p.distributor = rs.getString("distributor");
                p.retailer = rs.getString("retailer");
                p.status = rs.getString("status");
                p.assignedDelivery = rs.getString("assignedDelivery");
                p.lat = rs.getDouble("lat");
                p.lon = rs.getDouble("lon");
                p.flagged = rs.getBoolean("flagged");
                String timelineText = rs.getString("timeline");
                p.timeline = new ArrayList<>();
                if (timelineText != null && !timelineText.isEmpty()) {
                    String[] lines = timelineText.split("\\r?\\n");
                    for (String l : lines) if (!l.trim().isEmpty()) p.timeline.add(l);
                }
                // fallback to ensure at least one timeline entry
                if (p.timeline.isEmpty()) {
                    p.timeline.add(timestamp() + " - LOADED FROM DB - " + p.id);
                }
                products.put(p.id, p);
                customerProductListModel.addElement(displayForList(p));
                deliveryProductListModel.addElement(displayForList(p));
    ...
    private void loadLedgerFromDB() throws SQLException {
        String sql = "SELECT entry FROM ledger ORDER BY created_at ASC";
        try (Connection c = getConnection();
             PreparedStatement ps = c.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                String e = rs.getString("entry");
                if (e != null) {
                    ledger.add(e);
                }
            }
